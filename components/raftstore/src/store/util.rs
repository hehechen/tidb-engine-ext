...
impl RegionReadProgressRegistry {
    ...
    // same `LeaderInfo`
    pub fn handle_check_leaders<E: KvEngine>(&self, leaders: Vec<LeaderInfo>, coprocessor: &CoprocessorHost<E>) -> Vec<u64> {
...
impl RegionReadProgress
{
    ...
    pub fn update_applied<E: KvEngine>(&self, applied: u64, coprocessor: & CoprocessorHost<E>) {
        ...
        if let Some(ts) = core.update_applied(applied) {
            if !core.pause {
                ...
                coprocessor.on_update_safe_ts(core.region_id, TimeStamp::new(ts).physical(), 0)
            }
        }
    }
    ...
    pub fn merge_safe_ts<E: KvEngine>(&self, source_safe_ts: u64, merge_index: u64, coprocessor: &CoprocessorHost<E>) {
        let mut core = self.core.lock().unwrap();
        if let Some(ts) = core.merge_safe_ts(source_safe_ts, merge_index) {
            if !core.pause {
                self.safe_ts.store(ts, AtomicOrdering::Release);
                coprocessor.on_update_safe_ts(core.region_id, TimeStamp::new(ts).physical(), 1)
            }
        }
    }
    ...
    pub fn consume_leader_info<E: KvEngine>(&self, mut leader_info: LeaderInfo, coprocessor: &CoprocessorHost<E>) -> bool {
        let mut core = self.core.lock().unwrap();
